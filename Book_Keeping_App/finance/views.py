...
def emi_add(request):
    ...
    return render(request, 'finance/emi_form.html', {'form': form, 'action': 'Add'})
        ...
        if paid_by == 'split_person':
            ...
            if is_full_debit and emi.split_with and emi.split_amount > 0:
                SplitDebt.objects.create(
                    user=request.user,
                    person_name=emi.split_with.strip(),
                    debt_type='credit',
                    original_amount=emi.split_amount,
                    reason=f'EMI split share for {emi.title} ({emi.month_year})'
                )
                messages.success(request, f'{emi.title} marked PAID! ₹{debit_amount} debited from {pm_display}. Added ₹{emi.split_amount} to Credits for {emi.split_with}.')
            else:
...
def emi_edit(request, pk):
    ...
    return render(request, 'finance/emi_form.html', {'form': form, 'action': 'Edit'})
...
def debt_add(request):
    bens_count = Beneficiary.objects.filter(user=request.user).count()
    if bens_count == 0:
        messages.warning(request, 'Please add at least one Beneficiary first before creating debts or credits!')
...
def transaction_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 2. Process Beneficiary Splits & SplitDebt logs
            split_count = 0
            if is_split:
                split_ben_ids = request.POST.getlist('split_beneficiary_id[]')
                split_names = request.POST.getlist('split_person_name[]')
                split_amounts = request.POST.getlist('split_amount[]')
                split_types = request.POST.getlist('split_type[]')

                for ben_id, name, amt_str, stype in zip(split_ben_ids, split_names, split_amounts, split_types):
                    try:
                        amt = Decimal(amt_str)
                    ...
                    if ben_id and ben_id.isdigit():
                        ben_obj = Beneficiary.objects.filter(pk=int(ben_id), user=request.user).first()
                        if ben_obj:
                    ...
                    # Create TransactionSplit record
                    ...
                    # Log to SplitDebt for tracking owing status
                    SplitDebt.objects.create(
                        user=request.user,
                        beneficiary=ben_obj,
                        person_name=person_name,
...
def export_csv(request):
    ...
    for txn in all_txns:
        writer.writerow([txn.date.strftime('%Y-%m-%d'), txn.title, txn.get_category_display(), txn.get_payment_mode_display(), float(txn.amount), txn.description or ""])
...
def export_pdf(request):
    ...
    for txn in all_txns:
        td.append([txn.date.strftime('%Y-%m-%d'), txn.title[:35], txn.get_category_display(), f'Rs.{txn.amount}'])
    if not all_txns.exists():
